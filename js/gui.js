...
NGL.documentationUrl = 'http://nglviewer.org/ngl/api/manual/index.html'
var DMVDocumentationUrl = 'https://github.com/jaimergp/dmv'
var DMVLogo = "<img src='data:image/png;base64,' width='32' height='32' />"
...
NGL.MenubarWidget = function (stage, preferences) {
  var container = new UI.Panel()
  container.add(new UI.Html(DMVLogo).setClass('menu').setFloat('left').setMarginLeft('5px'))
  ...
  container.add(new UI.Panel().setClass('menu').setFloat('right').add(
    new UI.Text('InsiliChem DMV @ NGL ' + NGL.Version).setClass('title').onClick(function(){ 
        window.open(DMVDocumentationUrl, '_blank')
      })
    )
  )
...
NGL.MenubarFileWidget = function (stage) {
  ...
  function onOpenDriveOptionClick() {
    google.script.run.withSuccessHandler(openModalDrive)
                     .withFailureHandler(openModalError).showPicker();
  }

  function openModalDrive(html) {
    alert('In progress! In the meanwhile, use a URL query: ?fileid=<GoogleDriveFileID>&provider=drive')
    // uglipop({class: 'uglipop-div', //styling class for Modal
    //          source: 'html',
    //          content: html});
  }
  function openModalError(message) {
    alert('Could not open Google Drive picker... Error: ' + message);
  }
  ...
  var menuConfig = [
    createOption('Upload from PC', onOpenOptionClick, 'upload'),
    createOption('Import from Google Drive', onOpenDriveOptionClick, 'google-drive', iconPrefix='fab'),
    createOption('Import from Office.com', onOpenDriveOptionClick, 'microsoft', iconPrefix='fab'),
    createInput('Fetch PDB', onPdbInputKeyDown, 'download'),
    createDivider(),
    createCheckbox('Load as trajectory', false, onAsTrajectoryChange),
    createCheckbox('Load 1st model only', false, onFirstModelOnlyChange),
    createCheckbox('Load C-alpha only', false, onCAlphaOnlyChange),
    createDivider(),
    createOption('Screenshot', onScreenshotOptionClick, 'camera'),
    createOption('Export image...', onExportImageOptionClick, 'file-export')
  ]
  ...
  var optionsPanel = UI.MenubarHelper.createOptionsPanel(menuConfig).setWidth('220px')
...
NGL.MenubarViewWidget = function (stage, preferences) {
  ...
  function onToggleSidebarClick(){
    if (widget.sidebar.getWidth().slice(0, -2) <= 10) {
      widget.sidebar.setWidth('300px');
    } else{
      widget.sidebar.setWidth('10px');
    }
  }
  ...
  var menuConfig = [
    createOption('Toggle sidebar', onToggleSidebarClick),
    createDivider(),
    createOption('Light theme', onLightThemeOptionClick, 'sun'),
    createOption('Dark theme', onDarkThemeOptionClick, 'moon'),
    createDivider(),
    createOption('Perspective', onPerspectiveCameraOptionClick),
    createOption('Orthographic', onOrthographicCameraOptionClick),
    createOption('Stereo', onStereoCameraOptionClick, 'glasses'),
    createDivider(),
    createOption('Full screen', onFullScreenOptionClick, 'expand'),
    createOption('Center', onCenterOptionClick, 'bullseye'),
    createDivider(),
    createOption('Toggle spin', onToggleSpinClick, 'redo'),
    createOption('Toggle rock', onToggleRockClick, 'sync'),
    createDivider(),
    createOption('Get orientation', onGetOrientationClick, 'compass'),
    createOption('Set orientation', onSetOrientationClick, 'drafting-compass')
  ]
...
NGL.MenubarHelpWidget = function (stage, preferences) {
  ...
  var menuConfig = [
    createOption('Quickstart', onOverviewOptionClick, 'info'),
    createOption('Documentation', onDocOptionClick, 'book'),
    createDivider(),
    createOption('Debug', onDebugOnClick, 'bug'),
    // createOption('Debug off', onDebugOffClick),
    createDivider(),
    createOption('More options', onPreferencesOptionClick, 'cog')
  ]
...
NGL.OverviewWidget = function (stage, preferences) {
  ...
  headingPanel.add(
    new UI.Html(DMVLogo),
    new UI.Text('InsiliChem DMV').setFontStyle('italic').setMarginLeft('10px'),
    new UI.Html('&nbsp;&mdash;&nbsp;Quickstart')
  )
  headingPanel.add(
    new UI.Icon('times')
      .setCursor('pointer')
  ...
  listingPanel
    .add(new UI.Panel().add(new UI.Html("<h1>Opening files</h1>")))
    .add(new UI.Panel().add(new UI.Html("Easiest way from your local disk: drag the file to the canvas!")))
    .add(new UI.Break())
    .add(new UI.Panel().add(new UI.Html("Additionally, you can use the <i>File</i> menu. You can choose a file from your computer, fetch a PDB entry or import from Google Drive/Office.com [WIP].")))
    .add(new UI.Break())
    
    listingPanel
    .add(new UI.Panel().add(new UI.Html("Google Drive integration requires special permissions. First time you use this option, a popup will prompt for access. This will also register the app in your Drive services so you can use it to open files and attachments directly. Please <a href='" + DMVDocumentationUrl + "' target='_blank'>read here</a> for more information.")))
    .add(new UI.Break())
    
    listingPanel
    .add(new UI.Panel().add(new UI.Html("<h1>Usage</h1>")))
    .add(new UI.Panel().add(new UI.Html('<i>This app is only a slight modification of the NGL demo app. Credits to <a href="https://github.com/arose/" target="_blank">@arose</a> and everyone else involved. Following help message is his.</i>')))
    .add(new UI.Break())
  ...
  addIcon('trash', 'Deletes a component. Note that a second click is required to confirm the action.')
  ...
  listingPanel
    .add(new UI.Panel().add(new UI.Html(
      ...
      "<a href='" + NGL.documentationUrl + "' target='_blank'>NGL documentation pages</a>."
    )))
