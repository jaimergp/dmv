...
UI.MenubarHelper = {
  ...
  createOption: function (name, callbackHandler, icon, iconPrefix='fa') {
    var option = new UI.Panel()
    option.setClass('option')

    if (icon) {
      option.add(new UI.Icon(icon, prefix=iconPrefix).setWidth('20px'))
      option.add(new UI.Text(name))
    } else {
      option.setTextContent(name)
    }
    if (callbackHandler) option.onClick(callbackHandler)
  ...
  createInput: function (name, callbackHandler, icon, iconPrefix='fa') {
    var panel = new UI.Panel()
      .setClass('option')
      
    if (icon) panel.add(new UI.Icon(icon, prefix=iconPrefix).setWidth('20px'))
